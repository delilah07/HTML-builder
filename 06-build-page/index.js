...
fs.stat(projectFolder, (err) => {
  if (err) {
    makeDir(projectFolder);
    createHTML(templateFile, componentsFolder);
    mergeStyles(stylesFolder, projectFolder, 'style.css');
...
const makeDir = async (folder) => {
  fs.mkdir(path.join(folder), { recursive: true }, (err) => {
    if (err) {
      throw err;
    }
  });
    ...
    fs.readdir(compFolder, { withFileTypes: true }, (err, component) => {
      ...
      component.forEach((file) => {
        if (file.isFile() && path.extname(file.name) === '.html') {
          ...
          const fileInner = fs.createReadStream(
            path.join(compFolder, file.name),
            'utf-8'
          );
          ...
          fileInner.on('data', (data) => {
            ...
            fs.writeFile(indexFile, templateData, (err) => {
              if (err) {
                throw err;
              }
            });
